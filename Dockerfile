...
RUN apt-get -y update && apt-get install -y --no-install-recommends \
        ...
        timescaledb-2-postgresql-14 rabbitmq-server \
...
RUN echo "fsync = off" >>/etc/postgresql/14/main/postgresql.conf
RUN echo "full_page_writes = off" >>/etc/postgresql/14/main/postgresql.conf
...
RUN /home/enhydris/venv/bin/pip install --upgrade 'pip>=23,<24'
